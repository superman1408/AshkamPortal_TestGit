...
const Leave = () => {
  ...
  const UsFormatter = new Intl.DateTimeFormat("en-US");
  const fromDate = valueTo ? UsFormatter.format(valueTo) : "";
  const toDate = valueFrom ? UsFormatter.format(valueFrom) : "";

  // Calculate leave days excluding weekends
  const calculateTotalDays = () => {
    if (valueTo && valueFrom) {
      const from = new Date(valueTo);
      const to = new Date(valueFrom);
      let totalDays = 0;
      for (
        ...
        if (current.getDay() !== 6 && current.getDay() !== 0) totalDays++;
      }
      return totalDays;
    }
    return 0;
  ...
  const leaveTaken = calculateTotalDays();
  const availableLeave = totalLeave - leaveTaken;

  const handleSelect = (event) => setSelect(event.target.value);
  const leaveType = select;

  // Update subject automatically
  useEffect(() => {
    if (leaveType && fromDate && toDate) {
      const newSubject = `Applying for ${leaveType} from ${fromDate} to ${toDate}`;
      setSubjectFormat(newSubject);
  ...
  return (
    <Grid container className={classes.mainContainer} sx={{ mt: 2, gap: 2 }}>
      <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
        <Button onClick={handleGoBack} sx={{ color: "#16355d" }}>
          <ArrowBackIcon />
        </Button>
        <Typography
          variant="h4"
          sx={{ ml: 2, color: "#16355d", fontFamily: "Roboto" }}
        >
          Leave Section
        </Typography>
      </Box>
              ...
              <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) => {
                        const updatedSubject = e.target.value;
                        setSubjectFormat(updatedSubject);
                        setMailData({ ...mailData, subject: updatedSubject });
                      }}
                    />
                  </Grid>
                )}
                <Grid item xs={12}>
                  <TextField
                    ...
                    fullWidth
                    required
                    defaultValue={`\n\n\n\nThanks & Regards,\n${user.result.firstName} ${user.result.lastName} | ${user.result.department}\n${user.result.contactNumber}\nASHKAM ENERGY PRIVATE LIMITED`}
                    onChange={(e) =>
                      setMailData({
                        ...mailData,
                        requiredMessage: e.target.value,
